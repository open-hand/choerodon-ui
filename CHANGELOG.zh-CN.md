...
- 🌟 `configure`: 新增 selectBeforeCreateComboOption, uploadBatchMaxFileCount, attachment.previewTarget, lovModalAutoWidth, duplicateKey, uploadAlwaysShowActions 属性。
...
- 🌟 `<pro>TextField`: 新增 fieldFocusMode 属性。
...
- 💄 `Tabs`: 优化 tab 禁用样式。
- 💄 `<pro>Select`: 优化下拉选项 tooltip 延迟消失。
- 💄 `<pro>TextField`: 优化输入框长度不够导致多值 Tag 溢出时，显示 Tooltip。
- 💄 `<pro>Table`: 在 normal\bar\advancedBar 类型筛选条下，显示汇总条。
- 💄 `<pro>TextField`: 优化 range 模式下 suffix 占位宽度。
- 💄 `<pro>PerformanceTable`: 优化 flexGrow 列可以拖拽列宽。
...
- 💄 `<pro>Attachment`: 优化 Table 中必填显示样式。
...
- 💄 `Upload`: 优化表格中右对齐时的样式。
- 💄 `<pro>Lov`: 下拉模式支持树形选项。
- 💄 `<pro>Radio`: 优化 Radio CheckBox Switch 组件 float 布局的 label 样式。
...
- 💄 `<pro>Picture`: 优化图片预览器的 loading 样式。
...
- 🐞 `<pro>Table`: 修复开启虚拟滚动时，若存在较大横向滚动距离，重新查询数据后会渲染空白列的问题。
- 🐞 `<pro>Table`: 修复树形模式下存在不能勾选的子节点时，无法通过其父节点取消全选。
...
- 🌟 `<pro>Attachment`: 新增 enableDeleteAll, onPreview, pictureCardShowName, directory 属性。
...
- 💄 `<pro>Tooltip`: 优化触发弹窗显示的元素的位置变化时, 弹窗跟随变化。
...
- 🐞 `<pro>Table`: 修复数据分组模式下开启 showRemovedRow 属性，删除数据后单元格错位的问题。
- 🐞 `<pro>Table`: 修复粘贴空字符到数字类型单元格时会被转化成 0 的问题。
- 🐞 `<pro>Table`: 修复编辑器为多语言输入框时，无法通过回车跳转到下一行。
- 🐞 `<pro>Table`: 修复临时移除行的行号不正确的问题。
...
- 🌟 `<pro>Select`: 新增 scrollLoad, popupShowComboValue 属性。
