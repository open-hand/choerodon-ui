...
| lovQueryUrl | Lov query url or hook which return url | string \| ((code: string, lovConfig?: LovConfig, { dataSet, params, data, lovQueryDetail }) => string) | code => \`/common/lov/dataset/\${code}\` |
...
| textFieldPasteMaxLengthWarning | Warn when the pasted content exceeds the maximum input length, and you can customize the warning function. Priority is higher than inputLengthExceedWarning | boolean \| (props: { dataSet?: DataSet, field?: Field, name?: string, record?: Record, maxLength: number }) => void | - |
...
| highlightRenderer | 高亮渲染器, 需要配置 highlight 属性。表格中为非行内编辑的 可编辑单元格 渲染; 还可为表单组件高亮渲染。 | ({ title, content, dataSet, record, name, className, style }, element) => ReactNode |  |
...
| tooltipAutoPlacement | Automatically select the best tooltip position | boolean | false |
...
| uploadBatchMaxFileCount | The default maximum number of files in a single batch upload. If exceeded, the user will be prompted whether to continue; continuing uploads only the maximum number of files. Unconfigured or non-positive values impose no limit. | number |  |
...
| inputDecimalSeparatorFollowLang | 数字输入框输入时, 小数点分隔符是否跟随语言设置  | boolean |  |
...
| selectBeforeCreateComboOption | Callback before creating combo options, returning `true` allows creation, returning `false` prevents creation | (props: { text: string; options: DataSet; textField: string; valueField: string; }) => boolean | - |
...
| duplicateKey | Configuration for handling duplicate keys: Object or hook. `disable` controls whether to disable duplicates, `showWarning` determines whether to display a warning for duplicates, and both are defaulted to `true`. `componentName` includes Select, Lov, Table | { disable?: boolean; showWarning?: boolean } \| (componentName: string) => { disable?: boolean; showWarning?: boolean } | { disable: false, showWarning: true } |
| uploadAlwaysShowActions | Set the display status of the operation buttons for the Upload\Attachment component | boolean |  |
...
| text-field  | 输入类组件       |
| text-field-placeholder  | 输入类组件 placeholder       |
| undefined               | default                |
...
| previewTarget               | Target for attachment preview links. To preview in an iframe, set the iframe name to this value.               | string                             | 'attachment-preview' |
