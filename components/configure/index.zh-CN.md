...
| tooltipAutoPlacement | 自动选择最佳气泡框位置 | boolean | false |
...
| uploadBatchMaxFileCount | 单次批量上传的默认最大文件数量限制，超出会提示用户是否继续上传，继续则会截取最大数量上传；未配置或非正数时不限制 | number |  |
...
| previewTarget               | 附件预览链接的跳转目标；如需在 iframe 内预览，可将 iframe 的 name 设置为该值                | string                             | 'attachment-preview' |
