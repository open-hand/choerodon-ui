...
const WaterMark: React.FC<WaterMarkProps> = memo((props) => {
  ...
  const mutationObserver = (imgSrc: string) => {
    if (wrapperRef.current && !removeable) {
      const wrapperDom = getContainer && isElement(getContainer()) ? getContainer() : wrapperRef.current;
      // 监听浏览器控制台样式变化
      const styleStr = `position: absolute;left: 0;top: 0;width: 100%;height: 100%;z-index:${zIndex};pointer-events: none;background-repeat: repeat;background-size: ${gapX! + width!}px;background-image: url('${imgSrc}');opacity:${opacity}`;
