...
| [selection](/en/tutorials/table-select-record) | 选择的模式, 可选值: false 'multiple' 'single' | boolean \| string | multiple |    |
...
| --- | --- | --- | --- | --- |
| name | 字段名 | string |  |   |
| [type](/en/datasetapi/field-props/type) | 字段类型，可选值：boolean \| number \| string \| date \| dateTime \| time \| week \| month \| quarter \| year \| email \| url \| intl \| object \| attachment \| json \| bigNumber(1.5.1) | string | auto |  |
...
| [trueValue](/en/datasetapi/field-props/true-value) | 类型为 boolean 时，true 对应的值。当为数组时，默认使用数组第一个值，数组中的其他值会兼容显示 | boolean \|string \|number\|any[] | true | |
| [falseValue](/en/datasetapi/field-props/true-value) | 类型为 boolean 时，false 对应的值。当为数组时，默认使用数组第一个值，数组中的其他值会兼容显示 | boolean \|string \|number\|any[] | false |   |
...
| dateMode | 日期组件显示模式,可选值: `date` `dateTime` `time` `year` `month` `quarter` `week` | string | date  | 1.5.6 |
