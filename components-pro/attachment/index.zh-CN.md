...
| uploadImmediately | 是否立即上传，关闭后需要手动调用组件实例方法 `upload` 进行上传 | boolean | true |
| removeImmediately | 是否立即删除，关闭后需要手动调用组件实例方法 `remove` 进行删除 | boolean | true |
...
| alwaysShowActions | 设置操作按钮是否常显 | boolean |  |
