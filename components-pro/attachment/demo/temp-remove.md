...
const Children = ({ attRef }) => {
  ...
  const handleTempAttachmentsChange = React.useCallback((atts = []) => {
  ...
  return (
    <Form>
      ...
      <Button disabled={count === 0} onClick={removeFunc}>remove</Button>
      <Button disabled={count === 0} onClick={resetFunc}>reset</Button>
    </Form>
  );
}
