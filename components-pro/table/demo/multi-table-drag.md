...
使用 react-beautiful-dnd 扩展多表之间的行拖拽能力。开启 rowDraggable 和 multiTableRowDraggable 属性，同时使用 code 属性区分 Table 拖拽标识.

批量拖拽参考：https://react-beautiful-dnd.netlify.app/?path=/story/multi-drag--pattern

## en-US

Extend row dragging between multiple tables using react-beautiful-dnd.Enable the rowDraggable and multiTableRowDraggable attributes, and use the code attribute to differentiate between table drags.
