...
const TableWrapper: FunctionComponent<TableWrapperProps> = function TableWrapper(props) {
  ...
  const colGroup = useMemo((): ReactNode => {
    ...
    const cols = leafs.map(({ column, key }, index, array) => (
      <TableCol
        key={key}
        column={column}
        last={fullColumnWidth! && !overflowX && index === array.length - fixedColumnLength}
      />
    ));
  ...
  }, [leafs, customizable, rowDraggable, dragColumnAlign, hasPlaceHolder, lock, overflowX, fullColumnWidth]);
