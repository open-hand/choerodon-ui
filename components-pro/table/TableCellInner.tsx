...
const TableCellInner: FunctionComponent<TableCellInnerProps> = observer(function TableCellInner(props) {
  ...
  const colSpanStyle = useMemo(() => (colSpan && colSpan > 1 && (textAlign === ColumnAlign.right || textAlign === ColumnAlign.center)) ? { width: `calc(100% - ${pxToRem(30)})` } : {}, [textAlign]);
  const innerStyle = useMemo(() => {
    if (inAggregation) {
      return { ...prefixStyle, ...colSpanStyle };
    } else {
      return ({ textAlign: textAlign, ...prefixStyle, ...colSpanStyle })
    }
