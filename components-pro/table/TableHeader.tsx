...
export default class TableHeader extends Component<TableHeaderProps, any> {
  ...
  getTrs(): (ReactElement<TableHeaderRowProps> | undefined)[] {
    ...
    const rightLeafColumnsWidth = isStickySupport() && tableStore.overflowX ? tableStore.rightLeafColumnsWidth : 0;
    return rows.map<ReactElement<TableHeaderRowProps> | undefined>((row, rowIndex) => {
      if (row.length) {
        ...
        let leftWidth = 0;
        let rightWidth = rightLeafColumnsWidth + (hasPlaceholder ? measureScrollbar() : 0);
