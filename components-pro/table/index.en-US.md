...
| customizedUseHeader | 个性化面板是否使用列 header 的渲染结果，为 false 时面板展示列 title，无 title 则展示字段 label  | boolean | true |
...
| clipboard | 配置 Table 是否可复制粘贴。参考[配置项](#clipboard)。Lov 字段可自定义批量查询 [lovQueryBatchAxiosConfig](/components-pro/data-set-cn/#Field-Props) | Clipboard | { copy: false, paste: false } |
...
| customizedColumnProps | 个性化列的自定义列属性, 设置时需要注意: 个性化列仅有header部分, 仅有部分属性生效 |	ColumnProps \| ((defaultProps: ColumnProps) => ColumnProps) |	 |
| duplicateKey | Configuration object for duplicate primary keys(primaryKey); it can also be configured by component name through the global `duplicateKey` hook. `disable` controls whether duplicate records are disabled, and `showWarning` determines whether to display a warning for duplicates | { disable?: boolean; showWarning?: boolean } | [globalConfig.duplicateKey](/components/configure#API) |
...
| tagRenderer | 多值 Tag 渲染器 | ({ value, text, key, readOnly, invalid, disabled, onClose, className, inputBoxIsFocus, record, field }: TagRendererProps) => ReactNode |  |
...
| onQuery | 查询回调 | () => void |  |
| onReset | Reset callback, the type parameter being 'clear' indicates it is called when clearing the fuzzy search box, and being undefined indicates it is called when resetting the query conditions. | (type?: 'clear') => void |  |
