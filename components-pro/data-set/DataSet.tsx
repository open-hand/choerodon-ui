...
export default class DataSet extends EventManager {
  ...
  commitData(allData: any[], total?: number): DataSet {
    const { autoQueryAfterSubmit } = this.props;
    if (allData.length) {
      ...
      const { dirtyRecords } = this;
      if (dirtyRecords[0].length) {
        // 新增数据没有回写成功， 必须重新查询来获取主键
        if (autoQueryAfterSubmit) {
          warning(
            false,
            `The primary key which generated by database is not exists in each created records,
because of no \`__id\` field in each records from the response by \`submit\` or \`delete\` method.
Then the query method will be auto invoke.`,
        ...
        // 剩下未回写的数据使用本数据进行回写
        flatMap<Record>(dirtyRecords).forEach(record => record.commit(record.toData(), this));
      }
