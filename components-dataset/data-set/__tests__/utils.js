 ...
 * @param {*} _start 最小值
 * @param {*} _end 最大值
 * 只传一个值默认为 _end 并初始化 _start 为 0
 * @returns 返回一个随机整数 最小值默认为0
 */
 export const randomIdx = (_start, _end) => {
  let start = _end ? _start : 0;
  let end = _end ? _end : start;
  if(_start > _end) {
    start = _end;
    end = _start;
  }
...
export const randomData = (total) => {
  ...
  while(i < total) {
    ...
    const obj = {
      id: i,
      name: textArr[Math.round(Math.random() * textArr.length)] + textArr[Math.round(Math.random() * textArr.length)],
      sex: sexArr[Math.round(Math.random() * 2)],
      age: Math.ceil(Math.random() * 100),
      email: Math.round(Math.random() * (9999999 - 1000000) + 1000000) + "@qq.com",
      active: true
    };
    data.push(obj);
  }
