...
| labelWidth | 内部控件的标签宽度。如果为数组则分别对应每列的标签宽度。数组长度不够列数，以默认值补全, 响应式参考[Responsive](#响应式), 如果为auto，则根据内部label最大长度来对齐所有label。设置 minWidth 或 maxWidth 时，table 布局样式为 auto | number\| 'auto' \| ('auto' \| number)[] \| ({ minWidth?: number; maxWidth?: number }) \| object | [全局配置](/en/procmp/configure/configure) labelWidth | |
